...
const Step2 = () => {
  ...
  const step2 = useAppSelector((state) => state.signupSalon.step2);
  ...
  function updateStep2State() {
    ...
    dispatch(salonSliceActions.updateStep2(updatedData));
  }

  let formHandler = (e: FormEvent) => {
    e.preventDefault();
    updateStep2State();
    dispatch(salonSliceActions.setCurrentStep(3));
  };
        ...
        .then((res) => {
          ...
          if (res.status == 200) {
            ...
            if (imageType == "business") {
              ...
              dispatch(
                salonSliceActions.updateBusinessImages([...imageUrlsA, url])
              );
            } else if (imageType == "menu") {
              setImageUrlsB((curr) => [...curr, url]);
              dispatch(
                salonSliceActions.updateMenuImages([...imageUrlsB, url])
              );
            } else if (imageType == "cert") {
              setImageUrlsC((curr) => [...curr, url]);
              dispatch(
                salonSliceActions.updateCertImages([...imageUrlsC, url])
              );
            }
          }
        })
